...
func eject(storage types.Storage) i.ToolHandler {
	return i.NewTypedToolHandler(func(ctx context.Context, _ *mcp.CallToolRequest, args ejectArgs) (*mcp.CallToolResult, error) {
		...
		if record == nil {
			return i.NewToolResultError(fmt.Sprintf("Resource with ID '%s' not found in state", args.ResourceID)), nil
		}
		...
		if err := storage.DeleteState(ctx, args.ResourceID); err != nil {
			return i.NewToolResultError(fmt.Sprintf("Failed to eject resource from state: %v", err)), nil
		}
